...
const SEVERITY_CONFIG: Record<string, { icon: React.ElementType; bg: string; text: string; dot: string }> = {
  ...
  critical: { icon: XCircle,       bg: "bg-rose-100",  text: "text-rose-800",    dot: "bg-rose-700" },
  // legacy fallbacks
...
const EVENT_CATEGORIES = [
  ...
  { key: "applications",    label: "Applications",    desc: "Submissions and outcomes" },
...
export default async function LogsPage() {
  ...
  if (domainEvents && domainEvents.length > 0) {
    ...
    if (activityLog && activityLog.length > 0) {
      eventList = activityLog.map(e => ({
        id: e.id,
        event_type: (e as Record<string, unknown>).object_type as string ?? "event",
      ...
      eventList = (legacyEvents ?? []).map(e => ({
        id: e.id,
        event_type: (e as Record<string, unknown>).event_type as string ?? "event",
        status: (e as Record<string, unknown>).status as string ?? "pending",
        message: (e as Record<string, unknown>).message as string ?? undefined,
  ...
  const infoCount    = eventList.filter(e => (e.severity ?? e.status) === "info" || (e.severity ?? e.status) === "success").length
  const warningCount = eventList.filter(e => (e.severity ?? e.status) === "warning").length
  const errorCount   = eventList.filter(e => (e.severity ?? e.status) === "error" || (e.severity ?? e.status) === "critical").length
    ...
    <div className="hw-page">
      ...
      <div className="hw-page-header">
        <div>
          ...
          <p className="hw-page-subtitle">Every mutation across your jobs, documents, and pipeline.</p>
        </div>
      </div>
          ...
          {eventList.length === 0 ? (
            <div className="hw-empty">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1 max-w-sm">
                  Events appear here as you add jobs, run analysis, generate documents, and apply. Your complete event history will live here.
                </p>
              </div>
            ...
            <div className="hw-card overflow-hidden">
              <div className="divide-y divide-border/60">
                {eventList.map((event) => {
                  const severityKey = (event.severity ?? event.status ?? "info") as string
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {event.event_type !== label && (
                            <span className="text-[10px] text-muted-foreground/60 font-mono">{event.event_type}</span>
                          )}
                        </div>
                        {detail && (
