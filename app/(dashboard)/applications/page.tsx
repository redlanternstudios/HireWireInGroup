...
export default async function ApplicationsPage() {
  ...
  const applications = rawCast
    ...
    .filter(
      (a): a is ApplicationRow & { job: JobRef } =>
        !!a.job && !a.job.deleted_at,
    );
  ...
  const counts = {
    ...
    interviewing: applications.filter(
      (a) => displayStatus(a) === "interviewing",
    ).length,
    offered: applications.filter((a) => displayStatus(a) === "offered").length,
    rejected: applications.filter((a) => displayStatus(a) === "rejected")
      .length,
  };
  const conversionRate =
    applications.length > 0
      ? Math.round(
          ((counts.interviewing + counts.offered) / applications.length) * 100,
        )
      : null;
  ...
  return (
    <div className="hw-page">
      ...
      <div className="hw-page-header">
        <div>
          ...
          <p className="hw-page-subtitle">
            Every application you&apos;ve submitted — tracked from send to
            outcome.
          </p>
        </div>
      ...
      <div className="hw-metrics">
        ...
        <div className="hw-stat">
          <span className="hw-stat-value text-blue-500">
            {counts.interviewing}
          </span>
          ...
          {applications.length === 0 ? (
            ...
            <div className="space-y-6">
              {statusOrder.map((status) => {
                ...
                return (
                  <div key={status}>
                    <h2 className="hw-section-label mb-2">
                      {groupLabels[status]}
                    </h2>
                          ...
                          >
                            <div className="hw-card px-5 py-4 flex items-center justify-between gap-4">
                              ...
                              <div className="flex items-center gap-3 shrink-0">
                                <Badge
                                  variant="outline"
                                  className={`text-[10px] font-medium ${STATUS_CLASS[ds] ?? "status-draft"}`}
                                >
                                  {STATUS_LABEL[ds] ?? ds}
                                </Badge>
        ...
        <div className="hw-workspace-rail">
          <div>
            ...
            <div className="hw-panel p-4 space-y-3 text-xs text-muted-foreground">
              <p>
                Applications are created when you submit through the{" "}
                <Link
                  href="/ready-to-apply"
              ...
              <p>
                Outcomes (interviewing, offered, rejected) are updated from each
                job detail page.
              </p>
              <p className="text-[11px]">
                Each row here traces back to a confirmed submission — no silent
                state changes.
              </p>
            </div>
          </div>
