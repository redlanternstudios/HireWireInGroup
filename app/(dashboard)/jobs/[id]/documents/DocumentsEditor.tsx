...
export default function DocumentsEditor({
  ...
  const [resumeFormat, setResumeFormat] = useState<ResumeFormatId>(
    job.resume_format,
  );
  ...
  const handleReset = () => {
    if (
      !confirm(
        "Reset to the original generated version? Your edits will be lost.",
      )
    )
      return;
    startTransition(async () => {
        ...
        <section className="rounded-lg border border-border bg-card p-4">
          <div className="mb-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Recommended: {RESUME_FORMATS[job.recommended_resume_format].label}{" "}
              + {RESUME_FONTS[job.recommended_resume_font].label}
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              {job.recommended_resume_reason}
            </p>
          ...
          <div className="mt-5">
            ...
            <div className="flex flex-wrap gap-2">
              {RESUME_FONT_IDS.map((fontId) => {
                ...
                const recommended =
                  fontId === RESUME_FORMATS[resumeFormat].defaultFont;
                return (
                  <button
                    key={fontId}
                    type="button"
...
function PackageGate({
  ...
  return (
    <div className="flex items-center gap-3 rounded border border-border bg-background px-5 py-4">
      <span
        className="inline-flex h-2 w-2 rounded-full bg-muted-foreground"
        aria-hidden
      />
      <p className="text-sm text-muted-foreground">
        Status:{" "}
        <code className="rounded bg-muted px-1 text-xs">{packageStatus}</code>
      </p>
    </div>
  );
}
