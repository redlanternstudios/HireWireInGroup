...
async function getCoachContext() {
  try {
    ...
    const appliedJobs = evaluatedJobs.filter(
      ({ readiness }) => readiness.outcome === "applied",
    ).length;
    const withMaterials = evaluatedJobs.filter(
      ({ readiness }) =>
        readiness.checklist.resume || readiness.checklist.coverLetter,
    ).length;
    ...
    const urgentReady =
      recentEvents.find(
        (e) =>
          e.event_type === "readiness_changed" && e.payload?.can_apply === true,
      ) ?? null;
  ...
  const content = (
    ...
    >
      ...
      <div className="flex items-center gap-1">
        ...
        {href && (
          <ChevronRight className="h-2.5 w-2.5 text-muted-foreground/0 group-hover:text-muted-foreground/50 transition-colors" />
        )}
      </div>
    </div>
  );
        ...
        <aside className="hidden lg:flex flex-col w-72 xl:w-80 shrink-0 overflow-y-auto">
          ...
          <div className="shrink-0 bg-background p-4 border-b border-white/6">
            ...
            {ctx?.urgentReady && (
              <Link
                href={
                  ctx.urgentReady.job_id
                    ? `/jobs/${ctx.urgentReady.job_id}`
                    : "/ready-to-apply"
                }
              >
          ...
          {ctx?.recentEvents && ctx.recentEvents.length > 0 && (
            <div className="p-4 border-b border-border/70">
              ...
              <Link
                href="/logs"
                className="mt-3 block text-[10px] text-primary hover:underline font-medium"
              >
