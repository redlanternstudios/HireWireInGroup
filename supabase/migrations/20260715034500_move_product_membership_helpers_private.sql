-- Keep policy helpers outside the exposed public API schema.
...
create or replace function private.is_amina_member()
returns boolean
language sql
stable
security definer
...
as $$
  select (select auth.uid()) is not null
    and (
      exists (
        select 1 from public.amina_profiles ap
        where ap.id = (select auth.uid()) and ap.deleted_at is null
      )
      or exists (
        select 1 from public.circle_profiles cp
        where cp.user_id = (select auth.uid())
      )
    );
$$;

create or replace function private.is_hirewire_member()
returns boolean
language sql
stable
security definer
...
revoke all on function private.is_amina_member() from public;
revoke all on function private.is_amina_member() from anon;
grant execute on function private.is_amina_member() to authenticated;

revoke all on function private.is_hirewire_member() from public;
revoke all on function private.is_hirewire_member() from anon;
grant execute on function private.is_hirewire_member() to authenticated;

drop policy if exists "profiles_select" on public.circle_profiles;
create policy "profiles_select"
  on public.circle_profiles for select to authenticated
  using (
    private.is_amina_member()
    and (user_id = (select auth.uid()) or coalesce(is_private, false) = false)
  );

drop policy if exists "feature_flags_select_authenticated" on public.feature_flags;
create policy "feature_flags_select_authenticated"
  on public.feature_flags for select to authenticated
  using (private.is_hirewire_member());
...
begin
  if v_role <> 'service_role' then
    if v_uid is null or not private.is_hirewire_member() then return false; end if;
    if p_user_id is not null and p_user_id <> v_uid then return false; end if;
  end if;
  v_effective_user := coalesce(p_user_id, v_uid);
  select fo.enabled into v_override from public.flag_overrides fo
  where fo.flag_key=p_flag_key and fo.scope='user' and fo.scope_id=v_effective_user limit 1;
  if found then return v_override; end if;
  select fo.enabled into v_override from public.flag_overrides fo
  where fo.flag_key=p_flag_key and fo.scope='global' and fo.scope_id is null limit 1;
  if found then return v_override; end if;
  select ff.status into v_status from public.feature_flags ff where ff.flag_key=p_flag_key limit 1;
...
as $$
  ...
  where private.is_amina_member()
    and (
      s.author_id is null
      or not exists (
        select 1 from public.amina_user_blocks b
        where b.blocker_id=(select auth.uid()) and b.blocked_user_id=s.author_id
      )
    )
...
begin
  ...
  if not private.is_amina_member() then raise exception 'Amina membership required'; end if;
...
drop function public.is_amina_member();
drop function public.is_hirewire_member();
