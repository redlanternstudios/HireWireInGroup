...
import {
  Briefcase, Plus, ArrowRight, Target, Sparkles,
...
import { derivePriority, PRIORITY_LABEL, PRIORITY_COLOR, PRIORITY_SORT_WEIGHT } from "@/lib/jobs/priority"
...
function timeAgo(dateStr: string | null | undefined): { relative: string; date: string } {
...
type FilterChip = "all" | "needs_evidence" | "needs_materials" | "needs_review" | "ready_to_apply" | "high_fit" | "recently_added" | "stale"
...
function nextActionFor(job: EnrichedJob): { label: string; desc: string; href: string } {
  switch (job.displayStage) {
    case "inbox":
    case "analyzed":
      return { label: "View job", desc: "Review analysis", href: `/jobs/${job.id}` }
    case "needs_evidence":
      return { label: "Add missing evidence", desc: "Match key requirements", href: `/jobs/${job.id}/evidence-match` }
    ...
    case "stale":
      return { label: "Continue", desc: "Resume progress", href: `/jobs/${job.id}` }
    default:
...
function tagsFor(job: EnrichedJob): string[] {
  ...
  if (job.displayStage === "needs_evidence") {
    ...
    if (gaps.length > 0) tags.push(`Missing ${gaps.length} proof point${gaps.length > 1 ? "s" : ""}`)
...
function JobRow({ job, isLast }: { job: EnrichedJob; isLast: boolean }) {
  ...
  return (
    ...
    >
        ...
        <div className="min-w-0">
          ...
          {tags.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-1">
              {tags.map(tag => (
                <span key={tag} className="inline-flex items-center gap-0.5 text-[10px] px-1.5 py-px rounded bg-amber-50 text-amber-700 border border-amber-200 font-medium">
                  {tag}
                </span>
              ))}
            </div>
      ...
      {/* STATUS column */}
      <div className="min-w-0">
        <Badge variant="outline" className={cn("text-[10px] font-semibold px-2 py-0.5 whitespace-nowrap", stageColor)}>
          {stageLabel}
        </Badge>
        {job.displayStage === "needs_evidence" && (
          <p className="text-[10px] text-muted-foreground mt-1">
            {((job.evidence_map as Record<string, unknown> | null)?.score_gaps as string[] | null)?.length ?? 0} blockers
          </p>
        )}
      ...
      <div>
        {job.score !== null ? (
          <>
            <p className={cn("text-sm font-bold tabular-nums", fitColor)}>{job.score}%</p>
      ...
      <div>
        ...
        {time.date && <p className="text-[10px] text-muted-foreground">{time.date}</p>}
      </div>

      {/* NEXT ACTION column */}
      <div>
        <Link href={action.href}>
          <p className="text-xs font-semibold text-foreground hover:text-primary transition-colors">{action.label}</p>
...
function IntelligencePanel({ jobs, onAddJob }: { jobs: EnrichedJob[]; onAddJob: () => void }) {
  ...
  const todayQueue: { num: number; label: string; time: string; href: string }[] = []
  ...
  return (
    <div className="shrink-0 space-y-3" style={{ width: 240 }}>
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "hsl(var(--card))", border: "1px solid rgba(26,23,20,0.07)", boxShadow: "0 1px 3px rgba(26,23,20,0.04),0 3px 8px rgba(26,23,20,0.04)" }}>
        <div className="px-4 py-3 border-b border-border/60 flex items-center gap-2">
          <BarChart2 className="h-3.5 w-3.5 text-primary" />
          <p className="text-[11px] font-bold uppercase tracking-widest text-foreground">Pipeline Intelligence</p>
        </div>
        <div className="px-4 py-3 space-y-2.5">
          {[
            { label: "Active jobs",   count: active,      color: "bg-primary" },
            { label: "Needs action",  count: needsAction, color: "bg-amber-500" },
            ...
            <div key={row.label} className="flex items-center gap-2">
              ...
              <div className="flex-1 h-1.5 rounded-full bg-muted overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.max(row.count > 0 ? 8 : 0, Math.round((row.count / maxBar) * 100))}%` }}
                />
              </div>
              <span className="text-[11px] font-bold tabular-nums text-foreground w-4 text-right">{row.count}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {topJob && (
        <div className="rounded-xl overflow-hidden" style={{ background: "hsl(var(--card))", border: "1px solid rgba(26,23,20,0.07)", boxShadow: "0 1px 3px rgba(26,23,20,0.04),0 3px 8px rgba(26,23,20,0.04)" }}>
          <div className="px-4 py-3 border-b border-border/60 flex items-center gap-2">
            ...
            <p className="text-[11px] font-bold uppercase tracking-widest text-foreground">Top Priority</p>
          </div>
          <div className="px-4 py-3">
            <p className="text-xs font-semibold text-foreground leading-tight">{topJob.role_title ?? "Untitled"}</p>
            <p className="text-[10px] text-muted-foreground mt-0.5">{topJob.company_name ?? "—"}</p>
            <p className="text-[11px] text-foreground mt-2">
              {PRIORITY_LABEL[topJob.priority]} · {DISPLAY_STAGE_LABEL[topJob.displayStage]}
            </p>
        ...
        <div className="rounded-xl overflow-hidden" style={{ background: "hsl(var(--card))", border: "1px solid rgba(26,23,20,0.07)", boxShadow: "0 1px 3px rgba(26,23,20,0.04),0 3px 8px rgba(26,23,20,0.04)" }}>
          <div className="px-4 py-3 border-b border-border/60 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[11px] font-bold uppercase tracking-widest text-foreground">{"Today's Queue"}</p>
            </div>
            <Link href="/jobs">
              <span className="text-[11px] text-primary font-medium">View all</span>
            </Link>
          </div>
              ...
              <Link key={item.href} href={item.href}>
                <div className="px-4 py-2.5 hover:bg-muted/40 transition-colors">
                  <div className="flex items-start gap-2">
                    <span className="text-[10px] font-bold text-muted-foreground w-3 shrink-0 mt-px">{item.num}</span>
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "hsl(var(--card))", border: "1px solid rgba(26,23,20,0.07)", boxShadow: "0 1px 3px rgba(26,23,20,0.04),0 3px 8px rgba(26,23,20,0.04)" }}>
        <div className="px-4 py-3 border-b border-border/60">
          <p className="text-[11px] font-bold uppercase tracking-widest text-foreground">Quick Actions</p>
        </div>
        <div className="divide-y divide-border/60">
          {[
            { href: "#add", icon: Plus,     label: "Paste a new job",  desc: "Analyze a job description", onClick: onAddJob },
            { href: "/coach", icon: Sparkles, label: "Ask Coach",       desc: "Get personalized guidance", onClick: undefined },
          ].map(item => (
            item.onClick ? (
              <button key={item.label} onClick={item.onClick} className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-muted/40 transition-colors text-left">
...
export function JobsPipelineClient({ jobs: rawJobs }: { jobs: PipelineJob[] }) {
  ...
  const visible = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      switch (sortKey) {
        ...
        case "recently_updated": return new Date(b.updated_at ?? b.created_at).getTime() - new Date(a.updated_at ?? a.created_at).getTime()
        case "highest_fit":      return (b.score ?? -1) - (a.score ?? -1)
        case "needs_action_first": return PRIORITY_SORT_WEIGHT[a.priority] - PRIORITY_SORT_WEIGHT[b.priority]
        case "closest_ready": {
          const ord = ["inbox","analyzed","needs_evidence","ready_to_generate","package_drafted","needs_review","ready_to_apply"]
          return ord.indexOf(b.displayStage) - ord.indexOf(a.displayStage)
        }
        case "company_az":       return (a.company_name ?? "").localeCompare(b.company_name ?? "")
        case "stale_first":      return (b.staleness.daysSinceUpdate ?? 0) - (a.staleness.daysSinceUpdate ?? 0)
        default:                 return 0
      }
    })
  ...
  return (
    <div className="w-full px-5 py-4" style={{ maxWidth: 1280, marginInline: "auto" }}>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            ...
            {needsActionCount > 0 && (
              <> &middot; <span className="text-amber-600 font-semibold">{needsActionCount} need{needsActionCount === 1 ? "s" : ""} action</span></>
            )}
          </p>
        </div>
      ...
      {/* Add Job form */}
      {showAddJob && (
        <div className="mb-4 rounded-xl p-5" style={{ background: "hsl(var(--card))", border: "1px solid rgba(26,23,20,0.07)", boxShadow: "0 1px 3px rgba(26,23,20,0.04)" }}>
          <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-3">Analyze a Job</p>
          <JobInputForm />
        </div>
      )}
      ...
      <div className="grid grid-cols-6 gap-2.5 mb-4">
        {[
          { label: "Total",        value: jobs.length,        sub: `${jobs.length > 0 ? `↑ ${jobs.length} this week` : "None yet"}`,  color: "text-foreground",   subColor: "text-muted-foreground" },
          { label: "Active",       value: viewCounts.active,  sub: "No change",                 color: "text-foreground",   subColor: "text-muted-foreground" },
          { label: "Needs Action", value: needsActionCount,   sub: needsActionCount > 0 ? `↑ ${needsActionCount} this week` : "No change", color: needsActionCount > 0 ? "text-amber-600" : "text-foreground", subColor: needsActionCount > 0 ? "text-amber-500" : "text-muted-foreground" },
          { label: "Ready",        value: viewCounts.ready,   sub: "No change",                 color: "text-emerald-600",  subColor: "text-muted-foreground" },
          { label: "Applied",      value: viewCounts.applied, sub: "No change",                 color: "text-blue-600",     subColor: "text-muted-foreground" },
          { label: "Stale",        value: staleCount,         sub: "No change",                 color: staleCount > 0 ? "text-orange-500" : "text-foreground", subColor: "text-muted-foreground" },
        ].map(s => (
          <div
            key={s.label}
          ...
          >
            ...
            <p className="text-xs font-semibold text-foreground mt-1 uppercase tracking-wide">{s.label}</p>
      ...
      <div className="flex gap-4 items-start">
        ...
        {/* Left — jobs table */}
        <div className="flex-1 min-w-0 rounded-xl overflow-hidden" style={{ background: "hsl(var(--card))", border: "1px solid rgba(26,23,20,0.07)", boxShadow: "0 1px 3px rgba(26,23,20,0.04),0 3px 8px rgba(26,23,20,0.04)" }}>
          ...
          {visible.length > 0 && (
            <div
              ...
              style={{ gridTemplateColumns: "1fr 140px 120px 110px 160px 32px", background: "hsl(var(--muted)/0.3)" }}
            >
              {["Job", "Status", "Fit Score", "Last Updated", "Next Action", ""].map(col => (
                <p key={col} className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{col}</p>
              ))}
            </div>
          )}
          ...
          {visible.length === 0 ? (
            ...
            <div>
              {visible.map((job, i) => (
                <JobRow key={job.id} job={job} isLast={i === visible.length - 1} />
              ))}
        ...
        {/* Right rail */}
        <IntelligencePanel jobs={jobs} onAddJob={() => setShowAddJob(v => !v)} />
      </div>
    </div>
  )
}
