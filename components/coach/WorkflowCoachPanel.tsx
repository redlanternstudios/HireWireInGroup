...
export function WorkflowCoachPanel({
  recommendations,
  blockers,
  insights,
  momentum,
}: WorkflowCoachPanelProps) {
  return (
    <div className="space-y-3">
      {blockers.map((b, i) => (
        <CoachBlocker key={i} blocker={b} />
      ))}
      {recommendations.map((r) => (
        <EmbeddedCoachCard key={r.id} recommendation={r} />
      ))}
      {insights.map((ins, i) => (
        <CoachInsight key={i} insight={ins} />
      ))}
